...
function makeWorker(workerModuleDefinition, schema) {
  return `
(function() {
  var WorkerSchema = ${JSON.stringify(schema)};
  
  ${workerModuleDefinition}

  new WorkerInterface(${schema.className}, WorkerSchema);
})();
`;
}
...
function createWorker(options) {
  ...
  return rollup.rollup({
    entry: options.workerDefinition,
    ...
    const workerModuleDefinition = fs.readFileSync(options.workerFile, 'utf-8');
    fs.writeFileSync(options.workerFile, makeWorker(workerModuleDefinition, options.schema));
  });
}
